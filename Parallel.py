...
class Parallel(object):
    ...
    def extract(self, method='all', filename='default'):
        ...
        navigation_list = self.get_alphabet_suffix_list()
        ...
        with Pool(processes) as p:
            p.starmap(self.run_programme_extraction_per_char, zip(navigation_list, repeat(shared_list)))
        p.close()
        p.join()
        ...
        for character in shared_list:
            self.write_file(file_name, character)
    ...
    def get_alphabet_suffix_list(self):
        url = "http://bbc.co.uk/iplayer/a-z"
        browser = self.get_driver()
        self.get_page(browser, url)
    ...
    def get_page(self, browser, url, sleep=False, retry=False):
        ...
        try:
            ...
            if retry is False:
                print('retrying...')
                self.get_page(browser, url, retry=True)

    def run_programme_extraction_per_char(self, suffix, shared_list):
        ...
        if html is not None:
            ...
            for programme_box in tqdm(programme_selection, desc=suffix.split('/')[-1]):
                ...
                programme_box_json = self.parse_programme_box(programme_box)
                dictionary.update(programme_box_json)
                # get and extract initial programme page

                programme_microsite_url = self.parse_latest_episode(
                ...
                if programme_microsite_url is not None:
                    ...
                    programme_microsite_json = self.parse_programme_microsite(
    ...
    def parse_programme_box(self, programme_box):
        ...
        if synopsis is not None:
            ...
            if synopsis is not None:
                ...
                synopsis = programme_box.find(
                    'p', attrs={'class': 'list-content-item__synopsis typo typo--bullfinch gel-hide-lte@s'})
    ...
    def parse_latest_episode(self, browser, url):
        ...
        '''
        self.get_page(browser, 'http://bbc.co.uk' + url)
    ...
    def parse_programme_microsite(self, browser, url):
    ...
    def episodes(self, browser, html):
        ...
        if episodes_link is not None:
            ...
            self.get_page(browser, 'http://bbc.co.uk' + episodes_link)

            html_base = browser.execute_script("return document.body.innerHTML")

            html_base = BeautifulSoup(html_base, 'lxml')

            episodes_available.extend(self.episode_list_extractor(browser, html_base))
            ...
            if episode_pagination:
                ...
                for endpoint in page_links:
                    ...
                    episodes_available.extend(self.episode_list_extractor(browser, html))
            ...
            if next_on[-1].a:
                episodes_upcoming = self.upcoming_episodes(browser, 'http://bbc.co.uk'+next_on[-1].a['href'])

        # print(episodes_available)
        return episodes_available, episodes_upcoming


    def upcoming_episodes(self, browser, url):
                    ...
                    if program_info is not None:
                        ...
                        if program_title_info is not None:
                            ...
                            if program_link is not None:
                                ...
                                microsite_info = self.episode_microsite_extractor(
                                    browser, program_link, upcoming=True)
    ...
    def full_recommend(self, browser, url):
        ...
        if html is not None:
            ...
            if page_items is not None:
                ...
                for item in list_items:
                    ...
                    episode_microsite = self.episode_microsite_extractor(browser, 'http://bbc.co.uk' + link)
    ...
    def extract_childrens(self, browser, web_page):
        programme_dict = {}
    ...
    def programme_synopsis(self, html):
    ...
    def episode_list_extractor(self, browser, web_page):
        episodes_list = []
        ...
        if episodes_available_list is not None:
            ...
            for item in episodes_container_list:
                ...
                self.get_page(browser, episode_link)
                html = browser.execute_script("return document.body.innerHTML")
                ...
                if main_episode_info is not None:
                    ...
                    left_to_watch_dict = self.get_left_to_watch(html)
                    ...
                    featured_in_dict = self.get_featured_in(html)
                    episode_dict['episode'].update({'collection': featured_in_dict})

                    episode_broadcasts = self.get_boadcast_info(html)
                    episode_dict['episode']['broadcast'].update(
                        {'previous_broadcasts': episode_broadcasts})
    ...
    def episode_microsite_extractor(self, browser, url, upcoming=False):
        ...
        if main_episode_info is not None:
            ...
            if series_id is not None:
                ...
                episode_dict.update(
                    ...
                    if series_id.split('/')[-1] is not "ad" else series_id.split('/')[-2]})
                series_name = series_id_name[-1].get_text()
    ...
    def get_featured_in(self, web_page):
        ...
        featured_in = web_page.find(
            'div',
            attrs={'class': 'component component--box component--box-flushbody component--box--primary',
                'id': 'collections'})
